...
!
      ...
      lygo = isave(20)
!
!if(dabs(x(1))>3275786477704) write(*,*) "=====================001======================"
...
!if(dabs(x(1))>3275786477704) write(*,*) "=====================000======================"
!
      return
!
      end
...
!
     ...
     &                 iwhere(n), indx2(n), isave(44) !isave(23)
...
!
     ...
     &                 nint,nfgv,info,ifun,                             &
     &                 iword,nfree,nact,ileave,nenter,iddot
...
!if(dabs(x(1))>3275786477704) write(*,*) "=====================111======================"
            return
         endif
!
         call prn1lb(n,m,l,u,x,iprint,itfile,epsmch)
!if(dabs(x(1))>3275786477704) write(*,*) "=====================222======================"
!
!        Initialize iwhere & project x onto the feasible set.
!
         call active(n,l,u,nbd,x,iwhere,iprint,prjctd,cnstnd,boxed) 
!if(dabs(x(1))>3275786477704) write(*,*) "=====================333======================"
!
!         Write(*,*) 'inizializzazioni brade'
         iback  = isave(4)
         itail  = isave(8)
         ifun   = isave(15)
         iword  = isave(16)
...
!               call dcopy(n,t,1,x,1)
               x=t
!if(dabs(x(1))>3275786477704) write(*,*) "=====================444======================"
...
!if(dabs(x(1))>3275786477704) write(*,*) "=====================555======================"
!
      if (iprint .ge. 1) then
...
!if(dabs(x(1))>3275786477704) write(*,*) "=====================666======================"
      if (info .ne. 0) then 
...
!         call dcopy(n,t,1,x,1)
         x=t
!if(dabs(x(1))>3275786477704) write(*,*) "=====================999======================"
!         call dcopy(n,r,1,g,1)
         g=r
         f = fold
         if (col .eq. 0) then
...
!if(dabs(x(1))>3275786477704) write(*,*) "=====================00000======================"
         goto 1000
      endif
 777  continue
!
...
!      call dtrsl(wt,m,col,p(col+1),11,info)
      call dtrsl(wt(1:m,1:col),m,col,p((col+1):(2*col)),11,info)
      if (info .ne. 0) return
!
!     	solve D^(1/2)p1=v1.
      do 30 i = 1, col
...
!      call dtrsl(wt,m,col,p(col+1),01,info)
      call dtrsl(wt(1:m,1:col),m,col,p((col+1):(2*col)),01,info)
      if (info .ne. 0) return
!
...
!	 wmw = ddot(col2,wbp,1,v,1)
         ...
         Do iddot=1,col2
         xdot=v(iddot)
         wmc=wmc+c(iddot)*xdot
         wmp=wmp+p(iddot)*xdot
         wmw=wmw+wbp(iddot)*xdot
         EndDo
!
!           update p = p - dibp*wbp. 
...
!      if (col .gt. 0) call daxpy(col2,dtm,p,1,c,1)
      if (col .gt. 0) c(1:col2)=c(1:col2)+dtm*p(1:col2)
      if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
...
!
     ...
     &                 col2,pbegin,pend,dbegin,dend,upcl,iddot
      double precision temp1,temp2,temp3,temp4,xdot
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
!
...
!	       call dcopy(m-jy,wn1(jy+1,jy+1),1,wn1(jy,jy),1)
                    wn1(jy,1:jy)=wn1(jy+1,2:(jy+1))
! 	       call dcopy(m-jy,wn1(js+1,js+1),1,wn1(js,js),1)
                    wn1(js,(m+1):(m+jy))=wn1(js+1,(m+2):(m+jy+1))
! 	       call dcopy(m-1,wn1(m+2,jy+1),1,wn1(m+1,jy),1)
                    wn1(js,1:(m-1))=wn1(js+1,2:m)
  10        continue
         endif
!
...
!         call dtrsl(wn,m2,col,wn(1,js),11,info)
         call dtrsl(wn(1:m2,1:col),m2,col,wn(1,js),11,info)
  71  continue
!
...
!	       wn(is,js) = wn(is,js) + ddot(col,wn(1,is),1,wn(1,js),1)
               xdot=0.d0
               Do iddot=1,col
               xdot=xdot+wn(iddot,is)*wn(iddot,js)
               EndDo
 	       wn(is,js) = wn(is,js) + xdot
  74        continue
  72     continue
!
...
!            call dcopy(n,z,1,x,1)
            x=z
!if(dabs(x(1))>3275786477704) write(*,*) "=====================11111======================"
         else
            do 41 i = 1, n
	       x(i) = stp*d(i) + t(i)
!if(dabs(x(1))>3275786477704) write(*,*) "=====================22222======================"
  41        continue
         endif
      else
         task = 'NEW_X'
...
!     ************
!
      integer          j,pointr,kcopy,iddot
      double precision xdot,xdot1,xdot2
...
!      call dcopy(n,d,1,ws(1,itail),1)
      Do kcopy=1,n
      ws(kcopy,itail)=d(kcopy)
      EndDo
!
!      call dcopy(n,r,1,wy(1,itail),1)
      Do kcopy=1,n
      wy(kcopy,itail)=r(kcopy)
      EndDo
!
!     Set theta=yy/ys.
!
...
!	 ss(j,col) = ddot(n,ws(1,pointr),1,d,1)
         ...
         Do iddot=1,n
         xdot=d(iddot)
         xdot1=xdot1+xdot*wy(iddot,pointr)
         xdot2=xdot2+xdot*ws(iddot,pointr)
         EndDo
         sy(col,j)=xdot1
         ss(j,col)=xdot2
         pointr = mod(pointr,m) + 1
...
!     Compute wv = W'Zd.
!
!if(dabs(d(1))>3275786477704) write(*,*) "=====================fff======================"
      pointr = head 
      do 20 i = 1, col
     	 temp1 = zero
	 temp2 = zero
...
!if(dabs(wv(i)) > 3275786477704) write(*,*) "===ggg===",'wv=',wv(i),'i=',i
!if(dabs(wv(col+i)) > 3275786477704) write(*,*) "===ggg===",'wv=',wv(col+i),'i=',i,'col+i=',col+i
...
!      call dtrsl(wn,m2,col2,wv,11,info)
      call dtrsl(wn(1:m2,1:col2),m2,col2,wv(1:col2),11,info)
      if (info .ne. 0) return
      do 25 i = 1, col
	 wv(i) = -wv(i)
  25     continue
!      call dtrsl(wn,m2,col2,wv,01,info)
      call dtrsl(wn(1:m2,1:col2),m2,col2,wv(1:col2),01,info)
      if (info .ne. 0) return
!
...
!
     ...
     &                  + ws(k,pointr)*wv(js)
!if(dabs(d(i)) > 3275786477704) write(*,*) "===ggg===","wy=",wy(k,pointr),'wv=',wv(jy),'theta=',theta,'ws=',ws(k,pointr),'wv=',wv(js)
  30     continue
	 pointr = mod(pointr,m) + 1
  40  continue
      do 50 i = 1, nsub
	 d(i) = d(i)/theta
!if(dabs(d(1))>3275786477704) write(*,*) "=====================hhh======================"
  50  continue
!
...
!
	 ...
	 k = ind(i)
         dk = d(i)
!if(dabs(d(1))>3275786477704) write(*,*) "=====================iii======================"
	 if (nbd(k) .ne. 0) then
...
!
      if (alpha .lt. one) then
      	 ...
      	 if (dk .gt. zero) then
            x(k) = u(k)
!if(dabs(x(1))>3275786477704) write(*,*) "=====================ccc======================"
            d(ibd) = zero
!if(dabs(d(1))>3275786477704) write(*,*) "=====================jjj======================"
     	 else if (dk .lt. zero) then
            x(k) = l(k)
!if(dabs(x(1))>3275786477704) write(*,*) "=====================ddd======================"
	    d(ibd) = zero
     	 endif
      endif
      do 70 i = 1, nsub
	 k = ind(i)
	 x(k) = x(k) + alpha*d(i)
!if(dabs(x(1))>3275786477704) write(*,*) "=====================eee======================",'alpha=',alpha,'d(i)=',d(i)
...
!
      Implicit None
      character*(*) task
      integer isave(44) !isave(2)
      double precision f,g,stp,ftol,gtol,xtol,stpmin,stpmax
      double precision dsave(29) !dsave(13)
!     **********
!
!     Subroutine dcsrch
!
...
!
      temp =  0.d0 !etime(tarray) 
!
      ttime = 0.d0 !dble(tarray(1))
!
      return
!
      end
...
!====================== The end of dpofa ===============================
!
      subroutine dtrsl(t,lda,n,b,job,info)
!
      Implicit None
      integer lda,n,job,info
      double precision t(lda,n),b(n)
!
!
...
!
            ...
            Do j=n-1,1,-1
               somma=0.d0
                Do jj=j+1,n
                somma=somma+t(j,jj)*b(jj)
                EndDo
            b(j)=(b(j)-somma)/t(j,j) 
            EndDo
!
!        caso del sistema T^t.x=B
